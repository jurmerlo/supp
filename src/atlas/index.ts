 ...
 */
export function packAtlas(config: Config): void {
...
function packAtlases(config: Config): void {
  if (!config.atlases) {
    return;
  }
  ...
  for (const atlasConfig of config.atlases) {
    ...
    try {
      ...
      process.stdout.write(
        `Error: Failed to save atlas image for "${atlasConfig.name}": ${(error as Error).message}\n`,
      );
      continue;
    }
...
function padTilesets(config: Config): void {
  if (!config.tilesets) {
    return;
  }
  ...
  for (const tilesetConfig of config.tilesets) {
    ...
    try {
      ...
      process.stdout.write(
        `Error: Failed to save atlas image for "${tilesetConfig.name}": ${(error as Error).message}\n`,
      );
    }
  }
